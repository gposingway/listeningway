...
1. **User-toggleable**, off-by-default. ReShade uniforms and the overlay are always-on by definition; that's the addon's purpose. Network outputs open ports / sockets / external destinations and must be opt-in.
...
A lifecycle-only adapter. All consumers (internal always-on and toggleable network alike) implement it.
...
**One omission worth flagging**: there is no shared `consume(snapshot)` method. Each consumer chooses its own data acquisition pattern:
